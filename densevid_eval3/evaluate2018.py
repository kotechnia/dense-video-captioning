...
class ANETcaptions(object):
    ...
    def __init__(self, ground_truth_filenames=None, prediction_filename=None,
                 ...
                 prediction_fields=PREDICTION_FIELDS, verbose=False, no_lang_eval=False, lang = None, token=['PTB',None]):
        ...
        if not self.no_lang_eval:
            ...
            elif self.token[0] =='transformers':
                self.tokenizer = AutoTokenizer.from_pretrained(self.token[1])
            else:
                raise ValueError('bleu_token_type is PTB or transformer, or add your tokenizer')
    ...
    def import_prediction(self, prediction_filename):
        ...
        submission = json.load(open(prediction_filename, encoding=self.lang if self.lang else None))
    ...
    def import_ground_truths(self, filenames):
        ...
        for filename in filenames:
            gt = json.load(open(filename, encoding=self.lang if self.lang else None))
    ...
    def evaluate_tiou(self, tiou):
        ...
        for scorer, method in self.scorers:
            ...
            if self.token[0] == 'transformers':
                tokenize_res = {num : [' '.join(self.tokenizer.tokenize(caption['caption'])) for caption in captions] for num, captions in cur_res.items()}
                tokenize_gts = {num : [' '.join(self.tokenizer.tokenize(caption['caption'])) for caption in captions] for num, captions in cur_gts.items()}
            
            elif self.token[0] == 'PTB':
