...
class SODA:
    def __init__(self, data, soda_type="c", tious=None, scorer="Meteor", verbose=False, lang = None):
        #self.data = data
        self.lang = lang
    ...
    def build(cls, preds, gts, gt_vids, soda_type="c", tious=[0.0], scorer="Meteor", verbose=False):
        data = ANETCaptions(preds, gts, gt_vids, lang = self.lang)
        data.preprocess()
    ...
    def build_from_prediction(cls, preds, gt_files, soda_type="c", tious=[0.0], scorer="Meteor", verbose=False):
        data = ANETCaptions.from_prediction(gt_files, preds, lang=self.lang)
        data.preprocess()
        return cls(data, soda_type, tious, scorer, verbose)
