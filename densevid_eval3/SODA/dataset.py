...
class ANETCaptions:
    def __init__(self, preds, gts, gt_vid, verbose=False, lang=None):
        self.lang = lang
    ...
    @classmethod
    def from_load_files(cls, gt_file, pred_file, multi_reference=True, verbose=False, lang=None):
        gts, gt_vid = cls.load_ground_truth(gt_file, multi_reference=multi_reference, lang=lang)
        preds = cls.load_prediction(pred_file, lang=lang)
        gt_vid = cls.check_videos(gt_vid, preds.keys(),verbose=verbose)
        return cls(preds, gts, gt_vid, verbose=verbose)

    @classmethod
    def from_prediction(cls, gt_file, preds, multi_reference=True, verbose=False, lang=None):
        results = {}
    ...
    @staticmethod
    def load_ground_truth(filenames, multi_reference=False, verbose=False, lang=None):
        if verbose: 
        ...
        for filename in filenames:
            with open(filename, "r", encoding=lang if lang else None) as f:
                _gt = json.load(f) 
    ...
    @staticmethod
    def load_prediction(filename, verbose=False, lang=None):
