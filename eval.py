...
def main(opt):
    ...
    if opt.eval_mode == 'test':
        ...
        evaluate(model, criterion, postprocessors, loader, out_json_path,
                         logger, alpha=opt.ec_alpha, dvc_eval_version=opt.eval_tool_version, device=opt.eval_device, debug=False, skip_lang_eval=True, lang=opt.lang, bleu_token_type = opt.bleu_token_type)
