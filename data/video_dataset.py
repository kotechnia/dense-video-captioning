...
class Translator(object):
    def __init__(self, translator_json, vocob_size, type_ = "json"):
        self.vocab_size = vocob_size
        self.type_ = type_
        if self.type_ == 'json':
            self.vocab = json.load(open(translator_json, 'r', encoding='utf-8-sig'))
            print(self.vocab_size, len(self.vocab['word_to_ix'].keys()))
        ...
        elif self.type_ == 'model':
            self.vocab = spm.SentencePieceProcessor()
            self.vocab.load(translator_json)
            print('load translator, total_vocab: %d', self.vocab.vocab_size())
            


        else:
            self.vocab = AutoTokenizer.from_pretrained(translator_json)
            print('load translator, total_vocab: %d', self.vocab.vocab_size)
    ...
    def translate(self, sentence, max_len):
        ...
        if self.type_ == "json":
            ...
            res = np.array(
                    [0] + self.vocab.encode(sentence)[:max_len - 2] + [0]
                    )

        else:
            res = np.array(
                    [0] + self.vocab.encode(sentence,add_special_tokens = False)[:max_len - 2] + [0]
                                                                        )
        return res
    ...
    def rtranslate(self, sent_ids):
        ...
        if len(sent_ids):
            if self.type_ == "json":
                ...
                new_ids = [ int(idx) for idx in sent_ids]
                return self.vocab.decode(new_ids)
...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        self.anno = json.load(open(anno_file,'r', encoding=opt.lang if opt.lang else None))
        
        
        self.translator = Translator(translator_json, opt.vocab_size, opt.token_type)
