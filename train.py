...
def train(opt):
    ...
    while True:
        ...
        for dt in tqdm(train_loader, disable=opt.disable_tqdm):
            ...
            if losses_log_every == 0:
	            losses_log_every = 1  # add 
        ...
        if (epoch % opt.save_checkpoint_every == 0) and (epoch >= opt.min_epoch_when_save):
            ...
            eval_score, eval_loss = evaluate(model, criterion, postprocessors, val_loader, result_json_path, logger=logger, alpha=opt.ec_alpha, device=opt.device, debug=opt.debug, lang = opt.lang, bleu_token_type= opt.bleu_token_type)
